...
//内存池(32字节对齐)
_Pragma("data_alignment=64") u8 mem1base[MEM1_MAX_SIZE];													//内部SRAM内存池
_Pragma("data_alignment=64") u8 mem2base[MEM2_MAX_SIZE] @(0XC01F4000);					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
_Pragma("data_alignment=64") u8 mem3base[MEM3_MAX_SIZE] @(0X20000000);					//内部DTCM内存池
//内存管理表
u32 mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
u32 mem2mapbase[MEM2_ALLOC_TABLE_SIZE] @(0XC01F4000+MEM2_MAX_SIZE);	//外部SRAM内存池MAP
u32 mem3mapbase[MEM3_ALLOC_TABLE_SIZE] @(0X20000000+MEM3_MAX_SIZE);	//内部DTCM内存池MAP
//内存管理参数	   
